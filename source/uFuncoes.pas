...
DBXpress, SqlExpr, DBClient, Provider, uFrmAdmMain, DateUtils;
...
function mvcodigomin(campo,tabela:string):integer;
...
Function GetDiaVencimento(iDia : Integer) : String;
...
{  texto := 'Select min('+campo+') from '+tabela;
  ...
  with QrAux do
    try
      SQLConnection := dmAdmDados.dmAdmDados;
      sql.Add(texto);
      open;
...
Var
   M_NMPAST : String;
begin
    //M_NMPAST := dmAdmDados.cdsConfigCFG_NOME_PASTA.AsString;
    //
...
Function GetDiaVencimento(iDia : Integer) : String;
Var
   aData, aDataVenc : TDate;
   aDataStr, aDataVencimento : String;
Begin
     aData := Date();
     //
     aDataStr := DatetoStr(aData);
     // 99 / 99 / 9999
     aDataVencimento := InttoStr(iDia) + Copy(aDataStr, 3, 8);
     //
     aDataVenc := StrtoDate(aDataVencimento);
     //
     If (aDataVenc < Date()) Then
         aDataVenc := IncMonth(aDataVenc, 1);
     //
     Result := DatetoStr(aDataVenc);
End;
