...
begin
     ...
     Else
         cdsListaGrupoINICIOU.AsString := 'NÃO';
end;
...
begin
     ...
     if (cdsClientesAtrasoREC_VLPARC.AsFloat > 0) Then
      ...
      If (cdsClientesAtrasoCOT_POSICAO.AsString = 'A') Then
         cdsClientesAtrasoSTATUS.AsString := cdsClientesAtrasoCOT_POSICAO.AsString + '- Ativo-Ñ-Contem';
      ...
      If (cdsClientesAtrasoCOT_POSICAO.AsString = 'I') Then
         cdsClientesAtrasoSTATUS.AsString := cdsClientesAtrasoCOT_POSICAO.AsString + '- Inativo-Ñ-Contemp.';
      If (cdsClientesAtrasoCOT_POSICAO.AsString = 'P') Then
         cdsClientesAtrasoSTATUS.AsString := cdsClientesAtrasoCOT_POSICAO.AsString + '- Outros-Ñ-Contemp.';
      ...
      If (cdsClientesAtrasoCOT_POSICAO.AsString = 'R') Then
         cdsClientesAtrasoSTATUS.AsString := cdsClientesAtrasoCOT_POSICAO.AsString + '- Não Contpl.';
end;
...
begin
      ...
      With cdsReciboLances2 do
        begin
             ...
             if (aTipoHis = 'CL') Then
                 FieldByName('HISTORICO').AsString := 'Crédito em Lance';
             if (aTipoHis = 'BL') Then
...
begin
    try
         ...
         Begin
              ...
              Else
                  Print(M_POSINI, Format('%-48s', [uFuncoes.PadC('FECHAMENTO DE CAIXA - 2ª VIA',48)]), True);
              Font.Style := [];
   ...
   try
         ...
         //
             ...
             { salta duas linhas}
              ...
              Else
                  Writeln(impressora, Format('%-48s', [uFuncoes.PadC('FECHAMENTO DE CAIXA - 2ª VIA',48)]));
              { Imprime e alinhado à esquerda dentro de uma área de 48 caracteres}
              Writeln(impressora, Format('%-48s', [Replicate('*',48 )]));
              { Imprime o nome da Loja}
          ...
          //
           with dmRelatorios.ppRepRecibo do
            begin
                 ...
                 dmRelatorios.lblLinha2.Caption := 'A importância R$ '+ FormatFloat('###,##0.#0', fValor)
     ...
     if (cdsMasterAtrasadas.State in [dsInternalCalc]) Then
      begin
            ...
            If (cdsMasterAtrasadasCOT_POSICAO.AsString = 'A') Then
               cdsMasterAtrasadasSTATUS.AsString := cdsMasterAtrasadasCOT_POSICAO.AsString + '- Ativo-Ñ-Contem';
            ...
            If (cdsMasterAtrasadasCOT_POSICAO.AsString = 'I') Then
               cdsMasterAtrasadasSTATUS.AsString := cdsMasterAtrasadasCOT_POSICAO.AsString + '- Inativo-Ñ-Contemp.';
            If (cdsMasterAtrasadasCOT_POSICAO.AsString = 'P') Then
               cdsMasterAtrasadasSTATUS.AsString := cdsMasterAtrasadasCOT_POSICAO.AsString + '- Outros-Ñ-Contemp.';
            ...
            If (cdsMasterAtrasadasCOT_POSICAO.AsString = 'R') Then
               cdsMasterAtrasadasSTATUS.AsString := cdsMasterAtrasadasCOT_POSICAO.AsString + '- Não Contpl.';
            //
